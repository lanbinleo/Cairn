...
- Account equity is a frontend-recomputed snapshot on the Account record (`equity` = initialBalance + Σ closed PnL); the widget shows balance + 1%/2% risk amounts, and the trades table's PnL% column divides PnL by equity before that trade (`equityBeforeByTrade`).
