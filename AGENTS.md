...
- Case card `barRef` is a per-UTC-day index (resets at 00:00). Display resolution (`resolveCaseCardTimes` in `lib/bar-time.ts`) is monotonic by creation order: the first BAR card anchors to its own creation day; a later card whose `barRef` resolves before the previous card's time bumps to the next UTC day; equal numbers share one bar. No manual day disambiguation for cross-day Cases.
- The capture widget follows the current-Case session model: the panel header states the destination Case, switching is on demand, and starting a new Case is the primary organizational action. The panel isolates keyboard events originating inside its Shadow DOM with a window-capture interceptor (`composedPath` check + stopPropagation, so even TradingView's capture-phase shortcut listeners never see them; preventDefault only for the widget's own submit/save keys) — typing, pasting, and IME composition stay normal.
