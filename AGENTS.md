...
- Widget card correction (0.2.4): each card in the list has an always-visible ✎ that opens an inline editor for `rawText` (typo fix; history is backend-side) and `barRef` (empty = clear). It calls `PUT /api/v1/cases/{caseId}/cards/{cardId}` with `{ rawText, barRef }` (`barRef: null` clears) and expects the updated card back. No backend route exists yet (planned 0.2.2): the widget detects 404/405/immutable and toasts an upgrade hint while keeping the edit open.
