...
- `scripts/cairn-case-widget.user.js`: Tampermonkey widget (Shadow DOM, GM_xmlhttpRequest, current-Case session header, per-phase checklist hint 「这张卡可以覆盖：…」, key-event isolation via stopPropagation so TradingView shortcuts never fire while typing in the panel). `cairn-case-widget.test.html`: GM-shim harness.
