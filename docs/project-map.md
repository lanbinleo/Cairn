...
| AI trade tag suggestions (0.3.4, 「AI 标签建议」) | same call as execution suggestions (`run_execution_suggestions` — vocabulary from `tagDefs` grouped by color semantics `tag_vocabulary_block`) · parser `ai.rs parse_trade_tags` (词表命中 + quote 逐字, ≤15) · blob `TradeCase.aiTagSuggestions` (fingerprint = name) · UI `components/case-tag-suggestions.tsx` (Trade 案例 tab, under AI 补录建议) |
...
| Card re-split (0.3.4, 菜单「AI 重拆此卡」) | command `resplit_case_card` → `lib.rs run_card_resplit` + `persist_resplit` (三段短锁；失败原卡不动；成功软删原卡、新卡 createdAt+1ms 落原位、entryDecision/barRef 首张继承、剔除指向原卡的建议) · UI: card `···` menu in `case-card-timeline.tsx` |
