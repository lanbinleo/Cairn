...
| AI trade tag suggestions (0.3.4, 「AI 标签建议」) | same call as execution suggestions (`run_execution_suggestions` + optional `instruction`, vocabulary from `tagDefs` grouped by color semantics `tag_vocabulary_block`) · parser `ai.rs parse_trade_tags` (词表命中 + quote 逐字, ≤15; instruction runs also accept names typed verbatim in the instruction, 0.3.6) · blob `TradeCase.aiTagSuggestions` (fingerprint = name) · UI `components/case-tag-suggestions.tsx` (Trade 案例 tab, under AI 补录建议, `AiRetryLink` teachable retry incl. collapsed state) |
...
| Summary emphasis markup (0.3.4) | `ai.rs sanitize_summary_markup` (**加粗** / !!红!! / ==绿==, 未配对/空/跨行/嵌套/超量剥记号保文字) · frontend `lib/summary-markup.ts` (same semantics) · rendered by `case-summary-card.tsx`; `fillNote` strips |
...
| Batch voice split (批量拆卡) | `lib.rs run_batch_split` + `ai.rs build_split_messages`/`parse_card_splits` (0.3.4 prompt v2: 粒度=独立心智事件, 逐 K 线评述按观察点分开) · widget 「拆卡」勾选 → `POST /cases/:id/cards/batch-split` |
| Card re-split (0.3.4; two-step preview 0.3.6, 菜单「AI 重拆此卡」) | commands `preview_case_card_resplit` → `lib.rs run_card_resplit_preview` (AI, nothing persisted) + `apply_case_card_resplit` → `run_card_resplit_apply` + `persist_resplit` (短锁；originalText 回传比对防并发编辑；成功软删原卡、新卡 createdAt+1ms 落原位、entryDecision/barRef 首张继承、剔除指向原卡的建议) · UI: card `···` menu in `case-card-timeline.tsx` opens the preview dialog (改字/BAR/并入上一张, <2 段禁确认) |
| Card text editing assist (0.3.7) | `draft_case_card_rewrite` → `lib.rs run_card_rewrite_draft` (草稿不落库, 数字保留+长度比校验) · `proofread_case_card` → `run_card_proofread` (oldText→newText 对, 上下文 `bound_trade_lines`) · parsers `ai.rs parse_rewrite_draft`/`parse_proofread` · UI: `···` menu「AI 重写」(草稿填进编辑原文编辑器) + 「AI 校对」→ `components/case-card-correction-dialog.tsx` (勾选/手动对/预览) · 套用纯函数 `lib/case-card-corrections.ts` |
...
| Frontend unit tests (vitest) | `lib/bar-time.test.ts`, `lib/execution-display.test.ts`, `lib/process-score.test.ts`, `lib/case-summary.test.ts`, `lib/format.test.ts`, `lib/metrics.test.ts` (+ fee net cases), `lib/fee.test.ts`, `lib/case-card-corrections.test.ts`, `lib/case-import-matching.test.ts`, `lib/summary-markup.test.ts`, `lib/trade-filters.test.ts` (`pnpm test`) |
...
- `trades/page.tsx`: trade list. `trades/new/page.tsx`: manual creation. `trades/[tradeId]/page.tsx`: trade detail — tabs 复盘/案例/评估 (keys stay `overview`/`case`/`trade`, default 复盘): 复盘 = chart (case markers clickable, 轨迹线/入场线 toggles) + 时间线 + sidebar (标签 / 结果 PnL·R·过程分(未评分→去评分链接)·均价·时长 / 交易备注常显 / 元信息 / Case 单行链接); 案例 = binding panel + `CaseSummaryCard` (generation entry) + AI 补录建议 + AI 标签建议 + shared timeline; 评估 = 结果事实 (+风险网格, R 口径 ⓘ) + 计划 vs 实际 (偏差列) + 过程分.
- `cases/page.tsx`: Case list/filter/create. `cases/[caseId]/page.tsx`: Case detail — Case 概要, AI 总结 (`CaseSummaryCard`, compact when bound), 新增 Card 表单（BAR 可留空自动提取）, 心路历程（含全部识别、AI 拟题、移动 Card、编辑原文/AI 重写/AI 校对、删除 Card、AI 识别与落款行）, sidebar AI 找 Trade + Trade Binding.
...
- `store.tsx`: context store — hydration (`loadLocalState → normalizeSnapshot`), mutations (`createCaseCard` (nullable barRef), `moveCaseCard`, `updateCaseCardText`, `updateCaseCardBarRef`, `deleteCaseCard`, `updateCaseCardAnalysis`, `analyzeCaseCard`, `draftCaseCardRewrite`/`proofreadCaseCard` (0.3.7, task kinds `rewrite`/`proofread`), `refreshCaseExecutionSuggestions` + status updates, `summarizeCase`, bindings, imports…), `aiTasks` (store-level busy/error for manual 总结/补录建议检查 — survives tab navigation), binding-time auto-title + auto execution-suggestions + open→closed auto-summary (`stateRef` freshness), Entry-memo plan prefill effects, account equity snapshot effect (net of fees since 0.3.7), `cairn://data-changed` debounce refresh.
- `local-db.ts`: Tauri invoke wrappers + browser fallbacks (`isTauriRuntime()`, `analyzeCaseCard`, `suggestCaseExecutions`, `summarizeCase`, `suggestBindings`, `draftCaseTitle`, `draftCaseCardRewrite`/`proofreadCaseCard` (0.3.7), `getAiSettings`/`saveAiSettings`, `getNetworkSettings`/`saveNetworkSettings` (0.3.2 代理), API status, AI providers incl. `AiModelConfig` multi-model list + `AiThinkingLevel`).
...
- `summary-markup.ts`: summary narrative 受限标注 parser (`parseSummaryMarkup`/`stripSummaryMarkup`, mirrors Rust sanitizer).
- `metrics.ts`, `fee.ts` (0.3.7 费率/计提/订单分类), `executions.ts`, `execution-display.ts`: metric helpers (`equityBeforeByTrade` for PnL%, `computeEquityMaByTrades`/`computeEquityMaByDays` MA overlays; `computeTradeMetrics(trade, rates?)` 净额 + `grossPnl`/`fees`), fee model, execution classification, display aggregation (iceberg: same-bar same-price same-side fills merge into one row, entries and exits).
- `case-card-corrections.ts` (0.3.7): AI 校对替换对的顺序套用纯函数（失败标注不静默跳过）.
...
- `lib.rs`: builder, command registration (incl. `analyze_case_card`, `preview_case_card_resplit`/`apply_case_card_resplit`, `suggest_case_executions`, `ai_summarize_case`, `ai_suggest_bindings`, `draft_case_title`, `draft_case_card_rewrite`/`proofread_case_card` (0.3.7, GUI-only), `get_ai_settings`/`save_ai_settings`, `fetch_ai_models`, API config commands, widget script commands `get_widget_script`/`check_widget_script_update` — bundled via `include_str!` + GitHub main check through the Contents API), `run_card_analysis` + `card_context` (background context assembly) + `bound_trade_lines` (绑定交易事实块, 与校对上下文共享) shared by manual command and REST auto-analysis, `run_execution_suggestions` (context + 机械 dedup + fingerprint status merge; 0.3.4 同轮产出 `aiTagSuggestions` via `tag_vocabulary_block`), `run_batch_split` + `batch_split_endpoint` (voice batch split; endpoint dispatched at the api server loop so `handle_request` stays GUI-free and test-linkable), `run_card_resplit` + `persist_resplit` (0.3.4 card re-split), `run_card_rewrite_draft`/`run_card_proofread` (0.3.7), attachment file IO, setup hook (starts api server thread, daily auto-backup).
...
- `paths.rs`, `diagnostics.rs`, `main.rs`: app-data paths, per-date structured logs (`logs/cairn-YYYY-MM-DD.log`, kept-open handle, 14-day prune, panic → day file; commands `read_logs`/`clear_logs`/`get_logs_dir`/`open_logs_dir`; AI 请求/响应日志在 `ai.rs` 内), entrypoint.
