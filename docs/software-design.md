...
- Batch split (0.3.0): the capture widget detects multi-bar dictation (≥2 explicit K-line anchors, or ≥1 anchor plus "下一根" progression; a manually typed BAR opts out) and posts the whole speech to `cards/batch-split` with a `clientRequestId`. The server asks the AI to split by K-line anchors — explicit anchors set `barRef`, "下一根" increments it, explicit re-mentions reset — then mechanically validates every segment as a verbatim ordered substring with monotonic barRefs (`parse_card_splits`), and creates the cards directly: no preview, fluency first; mistakes are cleaned up with edit/delete (F7a). Any failure (no provider, network, bad parse) degrades to one whole-text card — the raw speech is never lost. Replay with the same `clientRequestId` returns the previously created cards. The route is dispatched at the server-loop level (not in `handle_request`'s table) because it needs the AppHandle for AI — keeping the routing table free of GUI dependencies so the test binary links cleanly. The AI call blocks the single-threaded server (90s cap); the widget is the only client and is awaiting this submit anyway.
