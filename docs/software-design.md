...
An Account is a trading environment or ledger, not necessarily a broker account. It contains initial balance, currency, `kind` (`backtest` or `live`), optional fee rates `takerFeePct` / `makerFeePct` (percentage numbers: `0.05` = 0.05%; missing or 0 = no fee, see Metrics), and `feesDisabled` (0.3.7) — a temporary switch that returns stats to gross while keeping the rate config (toggle in the collapsed 手续费 section on the account detail page).
