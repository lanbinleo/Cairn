...
- Endpoints: `GET /api/v1/health`, `GET/POST /api/v1/cases`, `GET /api/v1/cases/:id`, `GET/POST /api/v1/cases/:id/cards`, `PUT /api/v1/cases/:id/cards/:cardId` (raw-text correction + barRef), `DELETE /api/v1/cases/:id/cards/:cardId` (soft delete, 0.3.0), `POST /api/v1/cases/:id/cards/batch-split` (voice batch split, 0.3.0), `POST /api/v1/bindings`, `DELETE /api/v1/bindings/:id`, `GET/POST /api/v1/case-tags`, `GET /api/v1/accounts` (with nested periods for capture context).
- Batch split (0.3.0): the capture widget detects multi-bar dictation (≥2 explicit K-line anchors, or ≥1 anchor plus "下一根" progression; a manually typed BAR opts out) and posts the whole speech to `cards/batch-split` with a `clientRequestId`. The server asks the AI to split by K-line anchors — explicit anchors set `barRef`, "下一根" increments it, explicit re-mentions reset — then mechanically validates every segment as a verbatim ordered substring with monotonic barRefs, **requires ≥85% character coverage of the raw text (a dropped sentence fails the whole parse)**, caps segments at 20, and creates the cards directly: no preview, fluency first; mistakes are cleaned up with edit/delete (F7a). Any failure (no provider, network, bad parse, low coverage) degrades to one whole-text card — the raw speech is never lost. Replay with the same `clientRequestId` returns the previously created cards and **rejects a different rawText (409, matching the POST /cards immutable semantics)**; the widget reuses the request id for same-text retries and uses a 200s timeout (server worst case 90s×2). The route is dispatched at the server-loop level (before the DB mutex is taken) because it needs the AppHandle for AI — keeping `handle_request` free of GUI dependencies so the test binary links cleanly, and `run_batch_split` holds the DB lock only in short phases (validate/replay → AI unlocked → persist) so GUI Tauri commands never block behind a split. The AI call still occupies the single-threaded REST server (the widget is the only client and is awaiting this submit anyway).
...
Produced by the SAME AI call as execution suggestions (one invocation per check, prompt `0.3.4-suggest-2`; both blobs share `analyzedAt`), so the trigger surface is unchanged: binding established (UI `createCaseBinding` / REST `spawn_auto_suggestions`, `autoSuggest` gate) plus the shared 重新检查 button. Tags attach to the bound Trade, not the Case — after 1:1 binding they are one thing.
...
### Card Re-Split (AI 重拆此卡, 0.3.4)

Review-time rescue for cards that swallowed multiple thoughts (production reality: intermediate-phase running commentaries up to 1700 chars). The card `···` menu (shared timeline → both Case page and Trade 案例 tab) offers AI 重拆此卡 behind one confirm; it re-runs the batch-split prompt (0.3.4 granularity v2: one independent mental event per card) on the stored `rawText`.

- Degradation policy is the inverse of batch split: no provider, network failure, failed parse, or a single-segment result → explicit error, the original card untouched (a destructive replacement must not lose `aiAnalysis`/`rawTextHistory` for nothing). Cards under 60 chars are rejected up front; a card edited or deleted during the AI call aborts the replace.
- Success path (`run_card_resplit` → `persist_resplit`, same three-phase short-lock discipline as batch split): the original is soft-deleted (attachments follow, identical to card delete), N new cards are created at `createdAt = original + i` (position preserved; a fresh `rs-` request id per run so upserts never resurrect soft-deleted rows), `entryDecision` and the first segment's `barRef` are inherited, and both suggestion blobs lose entries pointing at the original card. New cards then auto-analyze (`autoAnalyze` gate).
- GUI-only Tauri command (`resplit_case_card`); the failure reason shows inline near the card and in the AI task center (`kind: 'split'`). No REST/widget route yet.
