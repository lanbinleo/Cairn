...
The first Stage 5 slice runs on each CaseCard ("AI 整理"): the default provider receives the Card phase and raw text and returns structured JSON that is validated and stored on the Card as `aiAnalysis` — a versioned derived record (`schemaVersion`, `promptVersion`, `model`, `providerId`, `analyzedAt`; schema `0.2.1-schema-2` / prompt `0.2.1-prompt-2` since 0.2.1). Raw text is never rewritten; re-running replaces the analysis.

- Extraction targets: `barRef` proposal (backfills a missing Card `barRef`), span-quote `labels` across eleven fixed categories, and for Entry Cards the seven-field memo (direction, entry price, stop-loss, target, confidence, invalidation, rejected alternatives, plus optional emotion). Every quote must be a verbatim substring of the raw text; anything else is dropped during validation, and `missingFields` is derived mechanically from the memo rather than trusted from the model.
