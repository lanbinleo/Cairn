...
Review-time rescue for cards that swallowed multiple bars (production reality: intermediate-phase running commentaries up to 1700 chars). The card `···` menu (shared timeline → both Case page and Trade 案例 tab) offers AI 重拆此卡 behind one confirm (the app-wide confirm dialog since 0.3.5, no native `window.confirm`); it re-runs the batch-split prompt (0.3.5 granularity v3: bar-anchored — a card only opens at an explicit bar advance, unanchored commentary merges back) on the stored `rawText`. Over-split cards from the 0.3.4 era are the cleanup target: re-splitting a same-bar commentary card yields one segment → "拆不出多张", original untouched.
